...
class Circle : public Object
{
  public:
    ...
    void correctOverlap(particle::Circle &other)
    {
        ...
        if (overlap > 0)
        {
            // Move each particle apart proportional to their inverse mass (simple correction: half/half)
    ...
    // Removed redundant methods (updatePosition and updateVelocity) which only called updatePhysics(ts).
